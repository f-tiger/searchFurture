...
import { verifyLicense } from "./billing.js";
...
export async function handleGenerate(request, env) {
  ...
  // Paid users (valid license) skip the free per-IP/day quota and get the better model.
  const licenseToken = String(
    body.license || request.headers.get("x-brandloop-license") || ""
  ).trim();
  const licensed = licenseToken ? await verifyLicense(env, licenseToken) : false;
  ...
  try {
    const resp = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        // Paid users get the higher-quality model; free tier favors cost.
        model: licensed
          ? (env.GEN_MODEL_PRO || "claude-opus-4-8")
          : (env.GEN_MODEL || "claude-haiku-4-5"),
        max_tokens: 2000,
    ...
    return json({ ok: true, posts: posts, pro: licensed });
  } catch (e) {
