...
function timingSafeEqualHex(a, b) {
  // Hex is case-insensitive; normalize so an upstream that emits uppercase isn't rejected.
  a = (a || "").toLowerCase();
  b = (b || "").toLowerCase();
 ...
 */
async function mintLicense(env, { email, refs, plan, provider, subId }) {
  ...
  // 7-day reverse lookups for the success page (which id the provider returns varies)
  for (const ref of record.refs) {
    await env.LEADS.put("sess:" + ref, token, { expirationTtl: 60 * 60 * 24 * 7 });
  }
  // Durable subscription→token map so renewals/cancellations can find this license.
  if (subId) await env.LEADS.put("sub:" + subId, token);
  return token;
}

/** Revoke access when a subscription truly ends (expired / unpaid / refunded / disputed). */
async function revokeLicense(env, subId, reason) {
  if (!env.LEADS || !subId) return false;
  try {
    const token = await env.LEADS.get("sub:" + subId);
    if (!token) return false;
    const raw = await env.LEADS.get("lic:" + token);
    if (!raw) return false;
    const rec = JSON.parse(raw);
    rec.status = "canceled";
    rec.canceled_reason = reason || "";
    rec.canceled_at = new Date().toISOString();
    await env.LEADS.put("lic:" + token, JSON.stringify(rec));
    return true;
  } catch (e) {
    return false;
  }
...
const creem = {
  ...
  async parseWebhook(raw, headers, env) {
    ...
    // Initial purchase → mint the license.
    if (type === "checkout.completed") {
      const cust = obj.customer || {};
      const order = obj.order || {};
      const sub = obj.subscription || {};
      const subId = (sub && (sub.id || (typeof sub === "string" ? sub : ""))) || "";
      const email = cust.email || obj.customer_email || event.customer_email || "";
      const refs = [obj.id, obj.request_id, obj.checkout_id, order.id, subId].filter(Boolean);
      return { ok: true, sale: { email, refs, subId } };
    }

    // Subscription truly ended → revoke. Here the entity IS the subscription, so its id is obj.id.
    // (Not on "canceled"/"scheduled_cancel": those keep access until the paid period ends.)
    if (type === "subscription.expired" || type === "subscription.unpaid") {
      return { ok: true, revoke: { subId: obj.id || "", reason: type } };
    }
    // Money clawed back → revoke. Here the entity is a refund/dispute that REFERENCES a
    // subscription via obj.subscription (id string or expanded object) — NOT obj.id.
    if (type === "refund.created" || type === "dispute.created") {
      const sub = obj.subscription;
      const subId = sub ? (typeof sub === "string" ? sub : sub.id || "") : "";
      return { ok: true, revoke: { subId, reason: type } };
    }
...
const stripe = {
  ...
  async parseWebhook(raw, headers, env) {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      const refs = [obj.id, subId, obj.customer].filter(Boolean);
      return { ok: true, sale: { email, refs, subId } };
    }
    // Subscription canceled/ended → revoke.
    if (event.type === "customer.subscription.deleted") {
      return { ok: true, revoke: { subId: obj.id || "", reason: event.type } };
    }
    return { ok: true, ignore: true };
  },
};
...
async function runWebhook(impl, providerName, request, env) {
  ...
  // Subscription ended / refunded / disputed → revoke access.
  if (parsed.revoke) {
    try { await revokeLicense(env, parsed.revoke.subId, parsed.revoke.reason); } catch (e) {}
  ...
  try {
    await mintLicense(env, {
      email: parsed.sale.email, refs: parsed.sale.refs,
      subId: parsed.sale.subId, plan, provider: providerName,
    });
  } catch (e) {}
...
export async function handleLicense(request, env) {
  ...
  // Validation mode: ?token=... → confirm a license is real & active (server truth).
  const token = url.searchParams.get("token");
  if (token !== null) {
    return json({ ok: true, valid: await verifyLicense(env, token) });
  }
