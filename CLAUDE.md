...
worker.js                      # Worker：服务静态资源 + /api/lead /generate /checkout /creem-webhook /stripe-webhook /license /health + scheduled() cron
lib/ops.js                     # 自动化运营：/api/health 就绪自检 + Cron IndexNow 自动提交索引
docs/GO-LIVE.md                # 上线唯一需人工的 ~5 分钟步骤（自验证 runbook）
...
## 自动化运营层（`lib/ops.js` · 零人工 · 无需账号）

- **就绪自检**：`GET /api/health` 返回各能力开关与 `ready_for_first_sale`（不泄密）。
  店主填完密钥后打开即知是否可成交——把激活变成"自验证"，不靠猜。
- **SEO 自动索引**：`wrangler.toml [triggers] crons` 每天触发 `worker.js scheduled()`，
  调 `submitIndexNow()` 把 sitemap 全部 URL 推给 IndexNow（Bing/Yandex/Naver/Seznam）。
  **永久自动跑、零人工、零账号**（key 自生成、托管在本域 `/<key>.txt`）。免费额度即可。
